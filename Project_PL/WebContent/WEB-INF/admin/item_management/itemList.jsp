			...
			<div class="product list paging">
				...
				<c:choose>
					<c:when test="${fn:length(requestScope.list)==0 }">
							등록된 물품이 없습니다.<p>
							<a href="${initParam.rootPath }/admin/itemList.do">처음으로</a>
						</c:when>
					<c:otherwise>
						<table style="width: 500px" border="1">
							<tr color="red">
								<th style="text-align: center; background-color: #FAE0D4;">물품
									ID</th>
								<th style="text-align: center; background-color: #FAE0D4;">품명</th>
								<th style="text-align: center; background-color: #FAE0D4;">가격</th>
								<th style="text-align: center; background-color: #FAE0D4;">카테고리ID</th>
							...
							<c:forEach items="${requestScope.list }" var="product">
								<tr>
									...
									<td><a
										href="${initParam.rootPath}/admin/findByItemId.do?itemId=${product.itemId }&categoryName=${product.categoryName}&pageNo=${requestScope.pagingBean.currentPage}">${product.itemName}</a>
									</td>
								...
								<td colspan="7" align="center">
									<!-- Paging 처리 --> <!-- 
										...
										<c:when test="${requestScope.pagingBean.previousPageGroup }">
											<a
												href="${initParam.rootPath }/admin/itemList.do?pageNo=${requestScope.pagingBean.startPageOfPageGroup-1}">
												◀ </a>
										</c:when>
										<c:otherwise>
										 		◀
										...
										<c:choose>
											...
											<c:otherwise>
												<a
													href="${initParam.rootPath }/admin/itemList.do?pageNo=${page }">
													${page } </a>
											</c:otherwise>
										</c:choose>
											&nbsp;&nbsp;
										...
										<c:when test="${requestScope.pagingBean.nextPageGroup }">
											<a
												href="${initParam.rootPath }/admin/itemList.do?pageNo=${requestScope.pagingBean.endPageOfPageGroup+1}">
												▶ </a>
										</c:when>
				...
				<div class="search_box pull-left">
					<form name="productSearch"
						action="${initParam.rootPath }/admin/productsByItemName.do"
						method="post" onsubmit="return productSearchCheck();">
