					...
					<c:otherwise>
							<table style="width: 800px" border="1">
								...
								<c:forEach items="${requestScope.list }" var="product">
									<tr>
										<td>
											<a href="${requestScope.rootPath }/owner/storedAndReleased.do">${product.itemName }</a>
										</td>
										<td>${product.itemPrice }</td>
										<td>${product.itemCount }</td>
										<td>${product.itemExpirationDate }</td>
										...
										- PageGroup의 시작/끝페이지 번호 - 반복문 처리
									 	...
									 	<c:forEach begin="${requestScope.pagingBean.startPageOfPageGroup }" end="${requestScope.pagingBean.endPageOfPageGroup }" var="page">
											<c:choose>
												...
												<c:otherwise>
													<a href="${initParam.rootPath }/owner/product_list.do?pageNo=${page }"> 
														${page }
													</a>
												</c:otherwise>
											</c:choose>
